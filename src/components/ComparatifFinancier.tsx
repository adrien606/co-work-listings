      ...
      <Collapsible open={open} onOpenChange={setOpen}>
        ...
        <CollapsibleContent className="mt-4">
          <div className="bg-card border rounded-lg p-4">
            ...
            <div className="mt-6 bg-background rounded-lg p-5 border">
              <h4 className="text-sm font-semibold text-foreground mb-5">Comparaison visuelle</h4>

              {/* Mensuel */}
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Coût mensuel réel</p>
              <div className="space-y-2 mb-6">
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-muted-foreground">Bail classique</span>
                    <span className="font-semibold text-orange-600">{fmtEur(totalBail)} €/mois</span>
                  </div>
                  <div className="h-6 bg-muted rounded-full overflow-hidden">
                    <div
                      className="h-full bg-orange-400 rounded-full transition-all duration-500"
                      style={{ width: `${Math.min(100, (totalBail / Math.max(totalBail, prixPrestation)) * 100)}%` }}
                    />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-muted-foreground">Contrat de prestation</span>
                    <span className="font-semibold text-emerald-600">{fmtEur(prixPrestation)} €/mois</span>
                  </div>
                  <div className="h-6 bg-muted rounded-full overflow-hidden">
                    <div
                      className="h-full bg-emerald-400 rounded-full transition-all duration-500"
                      style={{ width: `${Math.min(100, (prixPrestation / Math.max(totalBail, prixPrestation)) * 100)}%` }}
                    />
                  </div>
                </div>
              </div>
              ...
              {/* 3 ans */}
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Coût total sur 3 ans</p>
              <div className="space-y-2 mb-4">
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-muted-foreground">Bail classique</span>
                    <span className="font-semibold text-orange-600">{fmtEur(coutTotal3ansBail)} €</span>
                  </div>
                  <div className="h-6 bg-muted rounded-full overflow-hidden">
                    <div
                      className="h-full bg-orange-400 rounded-full transition-all duration-500"
                      style={{ width: `${Math.min(100, (coutTotal3ansBail / Math.max(coutTotal3ansBail, coutTotal3ansPrestation)) * 100)}%` }}
                    />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-muted-foreground">Contrat de prestation</span>
                    <span className="font-semibold text-emerald-600">{fmtEur(coutTotal3ansPrestation)} €</span>
                  </div>
                  <div className="h-6 bg-muted rounded-full overflow-hidden">
                    <div
                      className="h-full bg-emerald-400 rounded-full transition-all duration-500"
                      style={{ width: `${Math.min(100, (coutTotal3ansPrestation / Math.max(coutTotal3ansBail, coutTotal3ansPrestation)) * 100)}%` }}
                    />
                  </div>
                </div>
              </div>
              ...
              {/* Frais d'entrée */}
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Frais d'entrée</p>
              <div className="space-y-2">
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-muted-foreground">Bail classique</span>
                    <span className="font-semibold text-orange-600">~{fmtEur(fraisEntreeBail)} €</span>
                  </div>
                  <div className="h-6 bg-muted rounded-full overflow-hidden">
                    <div
                      className="h-full bg-orange-400 rounded-full transition-all duration-500"
                      style={{ width: `${Math.min(100, (fraisEntreeBail / Math.max(fraisEntreeBail, fraisEntreePrestation)) * 100)}%` }}
                    />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-muted-foreground">Contrat de prestation</span>
                    <span className="font-semibold text-emerald-600">{fmtEur(fraisEntreePrestation)} €</span>
                  </div>
                  <div className="h-6 bg-muted rounded-full overflow-hidden">
                    <div
                      className="h-full bg-emerald-400 rounded-full transition-all duration-500"
                      style={{ width: `${Math.min(100, (fraisEntreePrestation / Math.max(fraisEntreeBail, fraisEntreePrestation)) * 100)}%` }}
                    />
                  </div>
                </div>
              </div>
